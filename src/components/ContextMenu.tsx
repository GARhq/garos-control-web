...
import { Play, Square, RotateCcw, Monitor, Trash2, Settings, Copy, Power, HeartPulse, User } from 'lucide-react';
...
const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, node, onClose }) => {
  ...
  const menuItems = isTerminal ? [
    { label: 'Console VNC', icon: Monitor, color: 'text-kve-accent' },
    { label: 'Telemetria & Saúde', icon: HeartPulse, color: 'text-kve-success' },
    { label: 'Ligar via WOL', icon: Power, color: 'text-amber-400' },
    { label: 'Reiniciar Estação', icon: RotateCcw, color: 'text-kve-warning' },
    { divider: true },
    { label: 'Mensagem Popup', icon: User },
  ] : isVm ? [
